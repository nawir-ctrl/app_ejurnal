<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-slate-200">Edit Kelas</h2></x-slot>
        ...
        <div class="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-8 shadow-2xl">
            <form action="{{ route('classrooms.update', $classroom->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ $classroom->name }}" required class="bg-slate-900 border border-slate-700 text-white rounded-lg focus:ring-blue-500 block w-full p-2.5">
