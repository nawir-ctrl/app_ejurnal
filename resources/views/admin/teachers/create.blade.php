            ...
            <div class="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-8 shadow-2xl">
                <form action="{{ route('teachers.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nip" value="{{ old('nip') }}" class="bg-slate-900 border border-slate-700 text-white rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required class="bg-slate-900 border border-slate-700 text-white rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        @error('name') <span class="text-red-400 text-xs mt-1">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-2">Nomor HP</label>
                        <input type="text" name="phone" value="{{ old('phone') }}" class="bg-slate-900 border border-slate-700 text-white rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-slate-700/50">
                        ...
                        <button type="submit" class="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 hover:bg-blue-500 rounded-lg shadow-lg shadow-blue-500/30 transition-colors">Simpan Data</button>
                    </div>
                </form>
            </div>
        </div>
